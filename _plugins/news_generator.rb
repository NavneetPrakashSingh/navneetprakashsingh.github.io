...
module Jekyll
    class CategoryPageGenerator < Generator
      ...
      def generate(site)
        ...
        time = Time.new
        uuid = time.month.to_s.rjust(2,'0')+"_"+time.day.to_s.rjust(2,'0')+"_"+time.year.to_s.rjust(2,'0')
        ...
        if File.file?(file_path)
            ...
            for json_response in JSON.parse(response.body)['articles']
                ...
                json_response["summary"] = get_summary(json_response["url"]).encode('UTF-8', :invalid => :replace, :undef => :replace, :replace => "")
